...
**`fetch_data.py`** — This python file pulls the raw data. It calls the CollegeFootballData.com (CFBD) API to get every regular-season game (2021–2025) for the 67 teams in a Power 4 conference (ACC, Big Ten, Big 12, SEC), along with each team's box-score stats for that game. It then writes everything to `team_game_stats.csv`. Something I noticed and fixed was some data is pulled was per-team rather than per-conference, because several current P4 teams — USC, UCLA, Washington, Oregon, Arizona, Arizona State, Utah, Colorado, Cal, Stanford — were still in the old Pac-12 through 2023, and filtering by current conference would have missed those years' games entirely.

**`analyze_metrics.py`** — This is the code that answers the assignment. It reads the `team_game_stats.csv` and tests 9 different offensive metrics: Points per Game, Total Yards per Game, Yards per Play, Yards per Pass Attempt, Yards per Rush Attempt, Third-Down Conversion %, Completion %, Turnovers per Game, and First Downs per Game. For every team-season, it splits that team's games into a first half and second half, then computes each metric on both halves. Lastly, it prints the ranked results based on r and r^2.
...
- **r close to 1** means strong relationship.
- **r close to 0** means basically no relationship.
...
1. **Total Yards per Game (r = 0.606, R² = 0.368)** — This was the most reliable metric I tested. It collects enough plays per half-season that random single-play variance washes out, so a team's early-season yardage output is a good predictor of its later-season output.
